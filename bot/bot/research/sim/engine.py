"""Event-driven simulator (P3A). Replays recorded (or synthetic) events through the SAME SessionEngine, strategies,
order manager, risk engine and ledger used live, with PaperVenue (queue-aware fills, latency) as the venue.
...
from bot.common.config import DNSession, MMSession, RiskLimitsCfg, SafetyPauseCfg
...
from bot.research.sim.events import Event
...
class SimConfig:
    ...
    latency: dict[Venue, LatencyModel] = field(default_factory=lambda: {Venue.ARCUS: LatencyModel.arcus(),
                                                                         Venue.LIGHTER_RH: LatencyModel.lighter()})
...
class Simulator:
    def __init__(self, sessions: list[MMSession | DNSession], markets: dict[Venue, dict[str, Market]],
    ...
    def apply(self, e: Event) -> None:
        ...
        if e.kind in ("book_snapshot", "book_delta"):
            ...
            if pv is not None:
                pay_px = (view.oracle if e.venue is Venue.ARCUS else view.index) or view.mid() or \
    ...
    async def _margin_check(self) -> None:
        for v, pv in self.venues.items():
            ...
            for a in acts:
                ...
                self.fills.append(f)
                self.ledger.on_fill(f, a.price, kind="liquidation")
    ...
    def result(self) -> SimResult:
        ...
        for e in self.engines:
            stats[e.sid] = {"ticks": e.stats.ticks, "actions": e.stats.actions, "rejects": e.stats.rejects,
                            "errors": e.stats.errors, "hedges": e.stats.hedges, "risk_events": len(e.stats.risk_events)}
