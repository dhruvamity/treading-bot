"""Queue-aware passive fill model for the paper venue.

Rules:
- On arrival (after latency) a post-only order that would cross the book is REJECTED (Arcus POST_ONLY_WOULD_CROSS). Otherwise its queue position = displayed size at its price (0 if it improves).
